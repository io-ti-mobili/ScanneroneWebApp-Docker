...
public class ScoringService {
    ...
    public ScoringResult calculate(
        WifiNetworkUploadDto incoming,
        WifiNetwork existing
    ) {
        if (existing == null) {
            int points =
                POINTS_NEW_NETWORK + accuracyBonus(incoming.estAccuracy);
            return new ScoringResult(points, true, false, false);
        }

        boolean improvedAccuracy = isAccuracyImprovement(
            incoming.estAccuracy,
            existing.getEstAccuracy()
        );
